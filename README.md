...
### 6. Open the API Tester

Navigate to the tests/ folder in your project directory.
Locate the file named api_tester.html.
Copy the full file path.
Open any web browser and paste the copied path into the address bar.
Upload an image to check its quality and view the results.

Note: Ensure your backend server is running before using the tester so that the page can communicate with the API.
 ```SE_mini_project/tests/api_tester.html```
